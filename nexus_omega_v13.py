...
BASE_PRICES = {
    # TIER DISCOVERY ($0.0005-0.001) — preço mínimo para gerar volume
    "/fear-greed":    0.0005,
    "/trust-hash":    0.0005,
    "/web-search":    0.0005,
    "/pyth-price":    0.0005,
    ...
    "/agent-market":  0.0005,
    "/geo-alpha":     0.001,
    # TIER BASIC ($0.002-0.005) — dados com valor agregado
    "/dex-screen":    0.002,
    "/jupiter-swap":  0.002,
    "/rugcheck":      0.002,
    "/pump-monitor":  0.002,
    ...
    "/tg-premium":    0.005,
    "/nansen-flow":   0.003,
    # TIER PRO ($0.005-0.02) — análise IA + relatórios
    "/deep-think":    0.005,
    "/relatorio":     0.005,
    "/agent-call":    0.005,
    "/onchain-credit":0.005,
    "/sec-filing":    0.005,
    "/arbitrage":     0.01,
    "/cross-chain":   0.01,
    "/whale-alert":   0.01,
    ...
    "/copytrade":     0.01,
    "/sanctions":     0.01,
    # TIER ALPHA ($0.01-0.02) — sinais de alta confiança
    "/alpha-signal":  0.01,
    "/insider-track": 0.015,
    "/mev-flow":      0.02,
}
...
# ============================================================================
# 3b. WALLET CONSISTENCY CHECK — v13 REVOLUTION
# ============================================================================
...
def _check_wallet_consistency():
    """Verifica consistência entre wallets e loga warnings críticos."""
    warnings = []

    # 1. Verificar se WALLET_SECRET_B58 gera o endereço correto
    env_secret = os.environ.get("WALLET_SECRET_B58", "").strip()
    env_sol_addr = os.environ.get("SOLANA_WALLET_ADDRESS", "").strip()

    if env_secret and env_sol_addr:
        try:
            test_w = SolanaWallet(env_secret)
            if test_w.solana_address != env_sol_addr:
                warnings.append(
                    f"⚠️  INCONSISTÊNCIA: WALLET_SECRET_B58 gera {test_w.solana_address[:16]}... "
                    f"mas SOLANA_WALLET_ADDRESS é {env_sol_addr[:16]}..."
                )
                warnings.append("   → Se sweep for para Binance, use o mesmo endereço em ambos.")
        except Exception as e:
            warnings.append(f"⚠️  Erro ao verificar wallet: {e}")

    # 2. Verificar se BASE_PAYTO_EVM é válido
    if ENABLE_BASE and BASE_PAYTO_EVM:
        if not BASE_PAYTO_EVM.startswith("0x") or len(BASE_PAYTO_EVM) != 42:
            warnings.append(
                f"⚠️  BASE_PAYTO_EVM parece inválido: {BASE_PAYTO_EVM}. "
                "Deve ser endereço Ethereum válido (0x + 40 hex chars)."
            )
        else:
            log.info(f"✅ Base EVM payTo configurado: {BASE_PAYTO_EVM}")

    # 3. Verificar sweep destination
    if BINANCE_ADDRESS:
        if WALLET.solana_address == BINANCE_ADDRESS:
            log.info("✅ Sweep destination = node wallet (auto-sweep)")
        else:
            log.info(f"ℹ️  Sweep: {WALLET.solana_address[:16]}... → {BINANCE_ADDRESS[:16]}... (Binance)")

    # 4. Verificar facilitator
    if USE_FACILITATOR:
        log.info(f"✅ Facilitator: {FACILITATOR_URL}")
    else:
        log.info("ℹ️  Facilitator: OFF — ative com X402_FACILITATOR=https://x402.org/facilitator")
    ...
    if warnings:
        log.warning("=" * 62)
        log.warning("⚠️  PROBLEMAS DE CONFIGURAÇÃO DETECTADOS:")
        for w in warnings:
            log.warning(f"   {w}")
        log.warning("=" * 62)
        log.warning("Resolva antes de receber pagamentos reais.")
...
# Registra os 36+ endpoints (v13 REVOLUTION)
ENDPOINT_HANDLERS = {
...
def bootstrap_trust():
    ...
    Como: O dono faz 3 self-payments de $0.001 (de outra wallet) neste endpoint.
    ...
    if request.method == "GET":
        return jsonify({
            ...
            "instructions":   [
                ...
                f"2. Envie $0.001 USDC para {WALLET.solana_address}",
                f"3. POST /bootstrap-trust com X-Payment header contendo tx signature",
                "4. Repita 3x para gerar trust score inicial",
                f"5. Verifique seu trust score em https://www.x402scan.com/"
            ],
    ...
    # Verifica on-chain (aceita qualquer valor >= $0.001)
    ok, info = SOL.verify_payment(tx_sig, 0.001, WALLET.solana_address, max_age=3600)
    if not ok:
    ...
    LEDGER.add_revenue("/bootstrap-trust", 0.001, tx_sig, payer, source="bootstrap", chain="solana")
...
def get_pricing():
    ...
    return jsonify({
        ...
        "version":   VERSION,
        "ts":        int(time.time()),
        "promo":     "Preços promocionais ativos — 80% off endpoints alpha (30 dias)",
        "tiers": {
            "free":  {"endpoints": ["/sample"], "price": "grátis", "limit": "ilimitado"},
            "basic": {"endpoints": [e for e, p in BASE_PRICES.items() if p <= 0.003], "price_range": "$0.001-0.003"},
            "pro":   {"endpoints": [e for e, p in BASE_PRICES.items() if 0.003 < p <= 0.05], "price_range": "$0.005-0.05"},
            "alpha": {"endpoints": [e for e, p in BASE_PRICES.items() if p > 0.05], "price_range": "$0.05-0.10"},
        },
        "endpoints": {ep: {"price_usdc": p, "desc": ENDPOINT_DESC.get(ep, "")} for ep, p in BASE_PRICES.items()},
...
def bazaar_manifest():
    ...
    manifest = {
        "name":        "Losbeto",
        "version":     VERSION,
        "description": "Multi-chain x402 AI swarm — Solana+Base+TON. 36 endpoints. Pay-per-call USDC.",
...
def manifest_mcp():
    ...
    return jsonify({
        ...
        "name":           "losbeto-v13",
        "description":    "Multi-chain x402 AI swarm (Solana+Base+TON). 36 endpoints. Dynamic PoI pricing. Trust-score aware.",
        "tools":          tools,
        "node": {
            "version": VERSION,
            "node_id": WALLET.node_id,
...
def llms_txt():
    ...
    lines = [
        f"# Losbeto v{VERSION}",
        "> Multi-chain AI swarm. Solana + Base + TON. 36 endpoints. Pay-per-call via x402.",
        ...
        "",
        "## Pagar (Multi-Chain)",
        f"Solana payTo: {WALLET.solana_address} (network: solana:{SOL_GENESIS})",
    ]
    if ENABLE_BASE and BASE_PAYTO_EVM:
    ...
    # Group by price tier
    tiers = {"DISCOVERY ($0.0005-0.001)": [], "BASIC ($0.002-0.003)": [], 
             "PRO ($0.005-0.01)": [], "ALPHA ($0.01-0.02)": []}
    for p, price in sorted(BASE_PRICES.items(), key=lambda x: x[1]):
        if price <= 0.001:
            tiers["DISCOVERY ($0.0005-0.001)"].append((p, price))
        elif price <= 0.003:
            tiers["BASIC ($0.002-0.003)"].append((p, price))
        elif price <= 0.01:
            tiers["PRO ($0.005-0.01)"].append((p, price))
        else:
            tiers["ALPHA ($0.01-0.02)"].append((p, price))
...
<div class="header">
  <div><h1>⚡ Losbeto <span class="badge">v13 REVOLUTION</span></h1>
...
<div class="footer">Atualização automática a cada 10s. Win-rate dirige preço (PoI). v13 REVOLUTION.</div>
<script>
async function reload(){
  ...
  document.getElementById("tbl").querySelector("tbody").innerHTML =
    rows.length ? rows.map(([ep,n])=>`<tr><td><code>${ep}</code></td><td>${n}</td><td>$${(j.prices[ep]||0).toFixed(4)}</td></tr>`).join("")
                : '<tr><td colspan=3 style="text-align:center;color:var(--muted)">Aguardando pagamentos... Use /bootstrap-trust para gerar transações iniciais.</td></tr>';
}
...
def autoregister_x402scan():
    ...
    try:
        ...
        # Wallet + sweep info
        log.info(f"💰 Recebe em: {WALLET.solana_address}")
        if BINANCE_ADDRESS:
        ...
        log.info("   7. Bootstrap:     POST /bootstrap-trust com 3 tx de $0.001")
        log.info("=" * 62)
