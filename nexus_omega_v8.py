...
# URL pública do nó — Railway injeta RAILWAY_PUBLIC_DOMAIN automaticamente
_rw_domain = os.environ.get("RAILWAY_PUBLIC_DOMAIN", "").strip()
PUBLIC_URL = os.environ.get("PUBLIC_URL",
    f"https://{_rw_domain}" if _rw_domain else "").strip().rstrip("/")
...
def _build_402(endpoint):
    ...
    base = _public_base()
    # ── x402 spec v2 — formato exato que x402scan valida ──────────────────
    # Ref: https://github.com/x402-foundation/x402
    # Campo obrigatório: "amount" (não maxAmountRequired)
    # Header obrigatório: X-PAYMENT-REQUIRED (não Payment-Required)
    # x402Version: 2
    payload = {
        "x402Version": 2,
    ...
    resp.headers["X-PAYMENT-REQUIRED"]  = b64      # header correto (spec v2)
    resp.headers["PAYMENT-REQUIRED"]    = b64      # alias para compat. v1
...
def openapi_spec():
    ...
    for p, price in PRICES.items():
        paths[p] = {"get": {
            ...
            "responses": {
                "200": {"description": "Pago com sucesso"},
                "402": {"description": "Payment Required (header Payment-Required)"},
            },
        }}
    return jsonify({
        "openapi": "3.0.3",
        "info": {"title": "NexusOmega", "version": VERSION,
