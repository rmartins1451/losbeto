...
class FacilitatorClient:
    ...
    def verify(self, payment_payload: dict, requirements: dict) -> Tuple[bool, str, dict]:
        ...
        try:
            ...
            r = requests.post(f"{self.url}/verify", json=body, headers=hdrs, timeout=15)
    ...
    def settle(self, payment_payload: dict, requirements: dict,
        ...
        try:
            ...
            r = requests.post(f"{self.url}/settle", json={
                ...
                "paymentPayload": pp,
                "paymentRequirements": requirements,
            }, headers=self._headers("POST", "/settle"), timeout=30)
            if not r.ok:
...
def _verify_payment(endpoint: str, payment_header: str):
    ...
    if (FACILITATOR or CDP_FAC) and payload:
        ...
        for req in candidates:
            ...
            fac = (CDP_FAC if (CDP_FAC and str(req.get("network", "")).startswith("eip155"))
                   else FACILITATOR)
            if not fac:
                continue
