...
class FacilitatorClient:
    ...
    def verify(self, payment_payload: dict, requirements: dict) -> Tuple[bool, str, dict]:
        try:
            ...
            if not r.ok:
                # v22 FIX: não descarta mais o corpo do erro — o facilitator manda
                # o motivo real da rejeição (schema, rede não suportada, etc.) e
                # antes isso era jogado fora, aparecendo só como "facilitator-400".
                body_preview = (r.text or "")[:400]
                log.warning(f"⚠️ facilitator/verify {r.status_code}: {body_preview}")
                return False, f"facilitator-{r.status_code}", {"raw": body_preview}
    ...
    def settle(self, payment_payload: dict, requirements: dict) -> Tuple[bool, dict]:
        try:
            ...
            if not r.ok:
                ...
                log.warning(f"⚠️ facilitator/settle {r.status_code}: {body_preview}")
                return False, {"error": f"facilitator-{r.status_code}", "body": body_preview}
            return True, r.json()
        except Exception as e:
    ...
    def get_svm_fee_payer(self) -> Optional[str]:
        ...
        if cached and (time.time() - cached_at) < 3600:
            return cached
        try:
            r = requests.get(f"{self.url}/supported", headers=self._headers(), timeout=10)
            if not r.ok:
                return cached
            data = r.json() if r.text else {}
...
def _verify_payment(endpoint: str, payment_header: str):
    ...
    used_structured_payload = "payload" in payload if payload else False
    ...
    if FACILITATOR and payload:
        ...
        # v22 FIX: se o payload é um envelope x402 estruturado (payment via
        # facilitator, esquema patrocinado), NÃO existe uma tx já transmitida
        # pra procurar on-chain — cair no fallback abaixo só produzia o
        # "tx-not-found" enganoso que escondia o motivo real (o `last_reason`
        # acima). Nesse caso, retorna o motivo real do facilitator direto.
        if used_structured_payload:
