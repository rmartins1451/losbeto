...
BASE_PRICES = {
    # TIER FREE → BASIC (entry $0.001-0.003) — máximo volume
    "/fear-greed":    0.001,
    "/regime":        0.002,
    "/mempool":       0.002,
    "/sentiment":     0.002,
    ...
    "/dex-screen":    0.003,
    "/ai-news":       0.002,
    # TIER PRO ($0.005-0.05) — dados com análise
    "/jupiter-swap":  0.005,
    "/rugcheck":      0.005,
    "/pump-monitor":  0.005,
    "/defi-yield":    0.005,
    ...
    "/geo-alpha":     0.03,
    "/sinais":        0.01,
    # TIER ALPHA ($0.04-0.10) — desconto 80% promocional 30 dias
    "/whale-alert":   0.05,
    "/smart-money":   0.05,
    "/copytrade":     0.05,
    "/agent-market":  0.005,
    "/sanctions":     0.05,
    "/alpha-signal":  0.04,   # era $0.20, promo 80% off → gerar primeiros pagamentos
    "/insider-track": 0.05,   # era $0.25, promo 80% off
    "/mev-flow":      0.10,   # era $0.50, promo 80% off
}
...
@app.route("/sample")
def sample_free():
    """FREE TIER — dados reais limitados, sem pagamento x402.
    Converte agentes em pagantes. Taxa típica free→pago: 5-15%."""
    try:
        ...
        return jsonify({
            ...
            "note": "Dados limitados. Para análise completa com IA, use /fear-greed ($0.001 USDC via x402)",
            "pricing_url": f"{_public_base()}/get-pricing",
            "upgrade":     f"{_public_base()}/.well-known/x402.json",
            "free_calls_remaining": "unlimited",
        })
    except Exception as e:
        return jsonify({"service": "Losbeto — Free Sample", "error": str(e), "ts": int(time.time())})
...
def bazaar_manifest():
    ...
    return jsonify({
        "name":        "Losbeto",
        "version":     VERSION,
        "description": "Multi-chain x402 AI swarm — crypto data, trading signals, DeFi analytics",
        "tags":        ["crypto", "trading", "defi", "solana", "x402", "ai-agents"],
        "url":         base,
...
@app.route("/sample")
def sample_free():
    """Free sample sem paywall. Tática freemium: conversão típica 5-15%."""
    try:
        coins = Market.top_coins(5)
    except Exception:
        coins = []
    return jsonify({
        "losbeto":    "Free Sample — upgrade para dados completos",
        "version":    VERSION,
        "ts":         int(time.time()),
        "sample": {
            "top3": [{"symbol": c.get("symbol","").upper(), "price_usd": c.get("current_price")}
                     for c in coins[:3]],
            "note": "Dados limitados. Endpoint pago retorna análise IA completa.",
        },
        "paid_preview": {p: f"${v:.3f} USDC" for p, v in list(BASE_PRICES.items())[:6]},
        "how_to_pay":   "Chame qualquer endpoint → receba 402 com preço → pague USDC → obtenha dados",
        "wallet":       WALLET.solana_address,
        "full_catalog": f"{_public_base()}/get-pricing",
    })

@app.route("/get-pricing")
def get_pricing():
    """Lista de preços — tool MCP gratuita, otimizada para discovery por agentes."""
    try:
        multiplier = LEDGER.get_poi_multiplier()
    except Exception:
        multiplier = 1.0
    return jsonify({
        "ts":             int(time.time()),
        "service":        "Losbeto",
        "version":        VERSION,
        "poi_multiplier": multiplier,
        "network":        f"solana:{SOL_GENESIS}",
        "asset":          "USDC-SPL",
        "payTo":          WALLET.solana_address,
        "free_endpoint":  f"{_public_base()}/sample",
        "endpoints": {
            p: {"price_usdc": round(v * multiplier, 6), "base_price": v,
                "description": ENDPOINT_DESC.get(p, p)}
            for p, v in BASE_PRICES.items()
        },
    })

@app.route("/bazaar.json")
def bazaar_manifest():
    """Manifesto Bazaar CDP — padrão emergente de discovery no ecossistema x402."""
    base = _public_base()
    resources = []
    for p, price in BASE_PRICES.items():
        resources.append({
    ...
    return jsonify({
        "name":        "Losbeto",
        "version":     VERSION,
        "description": "Multi-chain x402 AI swarm — Solana. 36 endpoints crypto, sinais e análise. Pay-per-call USDC.",
        "url":         base,
        "icon":        f"{base}/favicon.ico",
        "contact":     os.environ.get("CONTACT_EMAIL", ""),
        "tags":        ["crypto","trading","defi","solana","signals","alpha","x402"],
        "free_tier":   f"{base}/sample",
        "pricing":     f"{base}/get-pricing",
        "resources":   resources,
        "ts":          int(time.time()),
    })
