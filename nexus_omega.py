...
def _build_402(endpoint: str):
    ...
    # v21 FIX: payload dual v1+v2 para máxima compatibilidade com scanners
    ...
    # v21 FIX: WWW-Authenticate é o header STANDARD que o x402scan valida.
    # DEVE vir primeiro e no formato EXATO: x402 challenge="<base64>"
    # v21 FIX: múltiplos challenges para cada rede aceita (compat x402scan)
    challenges = []
    for acc in accepts:
        net = acc.get("network", "").replace(":", "_")
        challenges.append(f'{net}="{b64}"')
    if not challenges:
        challenges = [f'challenge="{b64}"']
    resp.headers["WWW-Authenticate"] = "x402 " + ", ".join(challenges)
...
ENDPOINT_HANDLERS = {
    ...
    "/agent-composable":  Brain.agent_composable,
    "/win-rate-verified": Brain.win_rate_verified,
    "/multi-chain-arbitrage": Brain.multi_chain_arbitrage,
}
