...
def _verify_payment(endpoint: str, payment_header: str):
    ...
    if (FACILITATOR or CDP_FAC) and payload:
        ...
        for req in candidates:
            ...
            bz = None
            if getattr(fac, "is_cdp", False):
                # v21.14: blob completo da spec Bazaar (info + schema + description)
                # — o JSON Schema alimenta a busca semântica e o ranking do catálogo.
                bz = {
...
<div class="demo-card">
  <h3>🧾 Vendas reais, verificáveis on-chain</h3>
  <p>Cada linha abaixo é um pagamento x402 liquidado via facilitator — clique e confira na blockchain. Transparência total: <code>GET /receipts</code> para o feed completo em JSON.</p>
  <table>
  <thead><tr><th>Quando</th><th>Endpoint</th><th>Valor</th><th>Tx on-chain</th></tr></thead>
...
def root():
    ...
    try:
        with LEDGER.lock, LEDGER._conn() as _c:
            cur = _c.execute(
                "SELECT ts, endpoint, amount, tx_sig, chain FROM revenue "
                "WHERE source='facilitator' AND tx_sig IS NOT NULL AND tx_sig != '' "
                "ORDER BY ts DESC LIMIT 8")
            for ts_, ep_, amt_, sig_, ch_ in cur.fetchall():
