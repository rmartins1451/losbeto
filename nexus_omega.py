...
def _build_402(endpoint: str):
    ...
    payload = {
        ...
        "upsell": {
            # v24.5: a alavanca nº1 de conversão — o 402 é a resposta mais servida
            # do nó (milhares/dia). Antes mandava o agente para previews genéricos
            # de OUTROS endpoints; agora entrega o preview grátis DESTE recurso,
            # transformando sondagem em avaliação sem sair da página.
            "free_preview": {
                "endpoint": f"{base}{endpoint}?preview=1",
                "cost": "0.00",
                "note": "Same resource, real data delayed ~15min. Evaluate before paying."},
            "try_free": [f"{base}{endpoint}?preview=1", f"{base}/sample",
...
log.info(f"💎 Premium Suite v24.4: {len(PREMIUM_SUITE)} produtos registrados")
...
<div class="footer">refresh 10s · win-rate dirige preço via PoI multiplier · Losbeto v__DASHV__</div>
<script>
const CATEGORY_MAP = {
...
async function reload(){
  ...
  const KNOWN = [
    [/x402scan|fuchss|x402-trust|uptime|pingdom|monitor/i, "🤖 scanner/monitor", "muted"],
    [/coinbase|cdp|bazaar|payai|facilitator/i,             "🏪 catálogo/facilitator", "muted"],
    [/agentcash/i,                                          "🧪 AgentCash (provável você)", "muted"],
    [/bot|crawler|spider|scrapy|curl|wget|python-requests|go-http|okhttp/i,
                                                            "⚙️ bot genérico", "muted"],
  ...
  document.getElementById("tbl_ua").querySelector("tbody").innerHTML =
    uas.length ? uas.map(u=>{
      let label="❓ desconhecido — POSSÍVEL CLIENTE", cls="amber";
