...
OPERATOR_WALLETS = {w.strip() for w in os.environ.get("OPERATOR_WALLETS", "").split(",") if w.strip()}
...
BASE_PRICES = {
    ...
    "/launch-risk":       float(os.environ.get("PRICE_LAUNCH_RISK", "0.350")),
    "/council":           float(os.environ.get("PRICE_COUNCIL", "0.490")),
    "/launch-sniper":     0.490,
    "/whale-dossier":     0.590,
    "/thesis-engine":     0.690,
    "/starter-pack":      1.000,
    ...
    try:
        with LEDGER.lock, LEDGER._conn() as _c:
            ...
            for ts_, ep_, amt_, sig_, ch_, payer_, src_ in cur.fetchall():
                ...
                is_op = (src_ == "bootstrap" or (payer_ or "") in OPERATOR_WALLETS)
        ...
        with LEDGER.lock, LEDGER._conn() as _c:
            ...
            for ts_, ep_, amt_, sig_, ch_, payer_, src_ in cur.fetchall():
                origin = ("operator-test"
                          if (src_ == "bootstrap" or (payer_ or "") in OPERATOR_WALLETS)
                          else "organic")
                out.append({
                    "ts": ts_, "endpoint": ep_,
