...
def _verify_payment(endpoint: str, payment_header: str):
    ...
    if FACILITATOR and payload:
        ...
        for req in ordered:
            # v21.3 FIX: o /verify do PayAI valida o schema de paymentRequirements
            # (x402 v1-style): exige maxAmountRequired (não "amount"), resource,
            # description e mimeType. Passar o accept cru causa 400.
            req_full = {
                "scheme":            req.get("scheme", "exact"),
                "network":           req.get("network"),
                "maxAmountRequired": req.get("maxAmountRequired") or req.get("amount", "0"),
                "resource":          f"{base_url}{endpoint}",
                "description":       ENDPOINT_DESC.get(endpoint, endpoint),
                "mimeType":          "application/json",
                "payTo":             req.get("payTo"),
                "maxTimeoutSeconds": req.get("maxTimeoutSeconds", 300),
                "asset":             req.get("asset"),
                "extra":             req.get("extra", {}),
            }
            ok, reason, vdata = FACILITATOR.verify(payload, req_full)
            if not ok:
            ...
            settled, sdata = FACILITATOR.settle(payload, req_full)
            if not settled:
