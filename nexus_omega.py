...
class FacilitatorClient:
    ...
    def verify(self, payment_payload: dict, requirements: dict) -> Tuple[bool, str, dict]:
        ...
        try:
            r = requests.post(f"{self.url}/verify", json=body, timeout=15)
    ...
    if FACILITATOR and payload:
        ...
        for req in candidates:
            ...
            if _key in tried:
                continue
            tried.add(_key)
            ok, reason, vdata = FACILITATOR.verify(payload, req)
            if not ok:
                last_reason = reason or "facilitator-rejected"
                log.warning(f"⚠️ FACILITATOR.verify falhou p/ {endpoint}: {last_reason} | vdata={vdata}")
                continue
            ...
            settled, sdata = FACILITATOR.settle(payload, req)
            if not settled:
