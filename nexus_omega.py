...
class FacilitatorClient:
    ...
    def _cdp_jwt(self, method: str, path: str) -> Optional[str]:
        """v21.13: JWT Ed25519 exigido pela CDP (API key v2). O secret é o
        base64 de 64 bytes (privada||pública); a seed são os 32 primeiros."""
        try:
            from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey
            import secrets as _secrets
            sec = CDP_API_KEY_SECRET.strip()
            if "BEGIN EC PRIVATE KEY" in sec:
                log.warning("CDP: secret em formato EC (legado) — gere uma API key "
                            "nova no formato Ed25519 (padrão atual do portal CDP).")
                return None
...
CDP_FAC = (FacilitatorClient(CDP_FACILITATOR_URL)
           if (CDP_API_KEY_ID and CDP_API_KEY_SECRET
               and "BEGIN EC PRIVATE KEY" not in CDP_API_KEY_SECRET)
           else None)
if CDP_FAC:
    log.info(f"🏪 CDP/Bazaar habilitado p/ Base: {CDP_FACILITATOR_URL}")
else:
    _cdp_reason = ("CDP_API_KEY_ID ausente" if not CDP_API_KEY_ID
                   else "CDP_API_KEY_SECRET ausente" if not CDP_API_KEY_SECRET
                   else "secret em formato EC legado — gere key Ed25519 no portal CDP"
                   if "BEGIN EC PRIVATE KEY" in CDP_API_KEY_SECRET else "motivo desconhecido")
    ...
    if (FACILITATOR or CDP_FAC) and payload:
        ...
        for req in candidates:
            ...
            if getattr(fac, "is_cdp", False):
                bz = {"info": {"input": {"type": "http", "method": "GET"},
                               "output": {"type": "json"}},
