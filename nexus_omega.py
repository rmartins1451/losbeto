...
"""
================================================================================
 LOSBETO v18.0.0-REVENUE-FIX — APEX
================================================================================
...
# v17: default facilitator agora aponta para produção (CDP) quando NETWORK_MODE=mainnet.
# Ordem de preferência: env explícito > CDP mainnet (se CDP_API_KEY_ID definido) > PayAI > x402.org (dev)
_fac_env = os.environ.get("X402_FACILITATOR", "").strip()
CDP_API_KEY_ID     = os.environ.get("CDP_API_KEY_ID", "").strip()
CDP_API_KEY_SECRET = os.environ.get("CDP_API_KEY_SECRET", "").strip()
if _fac_env:
    FACILITATOR_URL = _fac_env
elif CDP_API_KEY_ID and CDP_API_KEY_SECRET:
    FACILITATOR_URL = "https://api.cdp.coinbase.com/platform/v2/x402"
else:
    FACILITATOR_URL = "https://facilitator.payai.network"  # produção livre, sem keys
...
class LedgerV10:
    ...
    def replay_delete(self, h: str) -> bool:
        '''v18: permite limpar uma entrada do replay cache (para re-verificação).'''
        with self.lock, self._conn() as c:
...
class SolanaClientV10:
    ...
    def _post(self, method, params, timeout=12):
        last_err = None
        for attempt in range(len(self.rpcs) * 2):  # v18: 2x tentativas (round-robin 2x)
            ...
            try:
                ...
                if r.ok:
                    ...
                    return r.json()
                # v18: se rate limit (429), espera e retry
                if r.status_code == 429:
                    wait = 2 ** attempt + random.uniform(0, 1)
                    log.warning(f"RPC {rpc[:30]}... rate limited (429), esperando {wait:.1f}s")
                    time.sleep(wait)
                ...
                self.fail_count[rpc] += 1
                last_err = e
                # v18: backoff exponencial entre RPCs
    ...
    def get_tx(self, sig):
        # v18: timeout aumentado para 20s + logging
        ...
        result = r.get("result")
        if not result:
            log.warning(f"get_tx: RPC retornou resultado vazio para sig={sig[:20]}...")
        return result
...
class FacilitatorClient:
    ...
    def _headers(self):
        ...
        if self.is_cdp and CDP_API_KEY_ID and CDP_API_KEY_SECRET:
            # CDP usa Bearer JWT ou key/secret via header proprietário.
            # Formato aceito: X-CC-Api-Key / Authorization: Bearer <secret>
...
if FACILITATOR:
    ...
    if "cdp.coinbase.com" in FACILITATOR_URL and not (CDP_API_KEY_ID and CDP_API_KEY_SECRET):
        log.warning("⚠️  CDP facilitator requer CDP_API_KEY_ID + CDP_API_KEY_SECRET. Sem elas, chamadas vão retornar 401.")
...
def _verify_payment(endpoint: str, payment_header: str):
    ...
    v18 REVENUE-FIX:
    - Extração robusta do tx_sig (base64 x402 v2 OU signature pura Phantom)
    - Fallback on-chain sempre que facilitator retornar erro
    - Aceita tx com valor >= metade do preço (proteje contra arredondamentos)
    """
    ...
    # Direct on-chain (Solana) — v17: tolera pequenos arredondamentos (aceita >= 95% do preço)
    if chain == "solana":
...
def bootstrap_trust():
    ...
    # v18 FIX: threshold flexível — aceita >= 95% do valor ou mínimo $0.005
    # Isso corrige o bug onde txs Phantom com valores como $0.999999 eram rejeitadas
    threshold = max(0.005, 0.01 * 0.95)  # 0.01 é o preço do bootstrap, mas aceitamos quase zero
    ...
    if not ok and isinstance(info, str) and info == "tx-not-found":
        # v18: retry com espera — tx pode ainda estar sendo confirmada
            ...
            "post": {
                ...
                "parameters": [
                    {"name": "X-Payment", "in": "header", "required": True,
                     "description": "Transaction signature da Solana",
                     "schema": {"type": "string"}}
                ],
                "responses": {
...
# ============================================================================
# 14b. v18 REVENUE-FIX — ENDPOINTS REVOLUCIONÁRIOS
# ============================================================================
#
# Novos endpoints que diferenciam Losbeto dos demais nodes x402:
#
#  /scan-tx?sig=<signature>         — escaneia tx Solana e mostra se é válida
#                                      para o RECEIVE_ADDRESS deste node.
#                                      Detecta swaps e transfers, retorna delta.
#  /register-marketplaces           — ping simultâneo a x402scan/CDP Bazaar/PayAI
#                                      para acelerar indexação do manifest.
#  /reconcile-tx (POST)             — reconcilia lista de assinaturas manuais
#                                      (Phantom) no ledger sem passar por
#                                      /bootstrap-trust um por um.
#  /health/deep                     — diagnóstico profundo: RPC, facilitator,
#                                      wallet, ledger, upstream APIs.
#
# Todos públicos (leitura). /reconcile-tx exige DASH_TOKEN.

@app.route("/scan-tx")
def scan_tx():
    """v17: valida uma tx Solana contra RECEIVE_ADDRESS deste node.

    GET /scan-tx?sig=<signature>
    Retorna JSON com: found, delta_usdc, payer, receiver_match, block_time,
    is_valid_payment (bool), suggested_action.
    """
    sig = (request.args.get("sig") or "").strip()
    if not sig:
        return jsonify({"error": "missing sig query param",
                        "example": f"{_public_base()}/scan-tx?sig=<base58-signature>"}), 400
    if len(sig) < 60 or len(sig) > 100:
        return jsonify({"error": "invalid signature length", "got": len(sig)}), 400
    ...
    if not tx:
        return jsonify({
            "signature": sig,
            "found": False,
            "suggested_action": "Aguarde 30s após a tx e tente novamente. Confirme na Solana mainnet-beta.",
    ...
    # Detecta se é um pagamento válido para RECEIVE_ADDRESS
    ...
    return jsonify({
        ...
        "explorer":          f"https://solscan.io/tx/{sig}",
        "suggested_action":  (
            "POST /bootstrap-trust com X-Payment: <sig> para registrar no ledger."
            if is_valid else
            "Esta tx NÃO envia USDC para RECEIVE_ADDRESS deste node. Verifique se você usou o endereço correto."
        ),
    })
...
def reconcile_tx():
    """v17: reconcilia uma lista de tx signatures manuais (Phantom) no ledger.

    Body JSON: {"signatures": ["sig1", "sig2", ...], "endpoint": "/bootstrap-trust"}
    Header: X-Dash-Token: <DASH_TOKEN>
    """
    tok = request.headers.get("X-Dash-Token") or request.args.get("token") or ""
    if tok != DASH_TOKEN:
        return jsonify({"error": "forbidden", "hint": "pass X-Dash-Token header"}), 403
    ...
    results = []
    for sig in sigs[:50]:  # max 50 por chamada
        sig = str(sig).strip()
        try:
            ok, info = SOL.verify_payment(sig, 0.005, RECEIVE_ADDRESS, max_age=86400 * 30)
            if ok:
                ...
                if LEDGER.replay_check(h):
                    ...
                    continue
                LEDGER.add_revenue(endpoint, amount, sig, payer,
                                    source="reconcile", chain="solana")
                results.append({"sig": sig[:32] + "...", "status": "recorded",
                                 "amount": amount, "payer": payer[:16] + "..."})
            else:
                results.append({"sig": sig[:32] + "...", "status": "invalid",
                                 "reason": str(info)})
        except Exception as e:
    ...
    return jsonify({
        ...
        "results":    results,
        "totals":     {"total_usdc": stats["total_usdc"],
...
def register_marketplaces():
    """v17: dispara pings de indexação para todos os marketplaces x402 conhecidos.

    Não requer auth: apenas incentiva crawlers a re-visitarem /.well-known/x402.json.
    Retorna resultado por marketplace.
    """
    base = _public_base()
    manifest_url = f"{base}/.well-known/x402.json"
    targets = [
        {"name": "x402scan",       "url": "https://www.x402scan.com/api/resources",
         "method": "POST", "body": {"url": base}},
        {"name": "cdp-bazaar",     "url": "https://api.cdp.coinbase.com/platform/v2/x402/bazaar/resources",
         "method": "POST", "body": {"url": manifest_url}},
        {"name": "payai-registry", "url": "https://facilitator.payai.network/registry/servers",
         "method": "POST", "body": {"url": manifest_url}},
        {"name": "agentcash",      "url": "https://agentcash.dev/api/servers",
         "method": "POST", "body": {"url": manifest_url}},
        {"name": "mcpay",          "url": "https://mcpay.tech/api/register",
         "method": "POST", "body": {"url": manifest_url}},
    ]
    results = []
    for t in targets:
        try:
            ...
            results.append({"marketplace": t["name"], "status": r.status_code,
...
def health_deep():
    """v17: diagnóstico profundo — RPC, facilitator, wallet, ledger, upstream."""
    checks = {}
    # 1. Solana RPC
    try:
        r = SOL._post("getSlot", [])
        checks["solana_rpc"] = {"ok": bool(r and r.get("result")),
                                "slot": (r or {}).get("result", 0)}
    except Exception as e:
        checks["solana_rpc"] = {"ok": False, "error": str(e)}
    # 2. RECEIVE_ADDRESS USDC balance
    try:
    ...
    if FACILITATOR:
        try:
            ...
            checks["facilitator"] = {"ok": r.status_code < 500, "status": r.status_code,
                                     "url": FACILITATOR.url}
        except Exception as e:
    ...
    try:
        s = LEDGER.stats()
        checks["ledger"] = {"ok": True, "total_usdc": s["total_usdc"],
                            "paid_24h": s["paid_24h"], "buyers": s["buyers"]}
    except Exception as e:
    ...
    # 6. External APIs (best-effort)
    for name, url in [("coingecko", "https://api.coingecko.com/api/v3/ping"),
                       ("fear_greed", "https://api.alternative.me/fng/")]:
        try:
            r = requests.get(url, timeout=4)
            checks[f"upstream_{name}"] = {"ok": r.status_code == 200}
        except Exception as e:
            checks[f"upstream_{name}"] = {"ok": False, "error": str(e)[:80]}
...
# ============================================================================
# 14c. v18 REVENUE-FIX — ENDPOINTS DE DIAGNÓSTICO E CORREÇÃO
# ============================================================================

@app.route("/debug-tx")
def debug_tx():
    """v18: Diagnóstico completo de uma tx — mostra EXATAMENTE o que o verify_payment vê.

    GET /debug-tx?sig=<signature>
    Retorna: raw tx data, pre/post balances, inner instructions, logs,
    e análise passo-a-passo de por que passou ou falhou.
    """
    sig = (request.args.get("sig") or "").strip()
    if not sig:
    ...
    if not tx:
        return jsonify({
            ...
            "found": False,
            "diagnosis": (
                "TX não encontrada nos RPCs. Possíveis causas:\n"
                "1. A tx ainda não foi confirmada (aguarde 10-30s)\n"
                "2. Os RPCs públicos estão com rate limit\n"
                "3. A signature está incorreta\n"
                "4. A tx foi enviada para devnet em vez de mainnet"
            ),
            "action": "Aguarde 30s e tente novamente. Ou use /force-register-tx para registrar manualmente."
        }), 200
            ...
            if "Token" in prog or prog == "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA":
                ...
                if ix_type in ("transfer", "transferChecked"):
                    ...
                    m2_results.append({
                        ...
                        "amount_info": info.get("tokenAmount") or info.get("amount"),
                        "authority": info.get("authority", info.get("sourceOwner", "")),
                    })
    ...
    # Recomendação
    if ok:
        recommendation = "✅ Esta tx é VÁLIDA. Use POST /bootstrap-trust ou /force-register-tx para registrar."
    elif not tx:
        recommendation = "⏳ Tx não encontrada. Aguarde confirmação."
    elif err:
        recommendation = "❌ Tx falhou on-chain. Não pode ser usada."
    else:
        recommendation = "❌ Tx encontrada mas não envia USDC para RECEIVE_ADDRESS. Verifique o destinatário."
...
def force_register_tx():
    """v18: Registra uma tx manualmente NO LEDGER, bypassando verificação on-chain.

    Use SOMENTE quando:
    - Você CONFIRMA que a tx existe no Solscan (solscan.io/tx/<sig>)
    - O /debug-tx mostra que a tx é válida mas o RPC falha
    - Você quer registrar vendas manuais que já foram confirmadas

    Header: X-Dash-Token: <DASH_TOKEN>
    Body JSON: {
        "signature": "base58-signature",
        "amount_usdc": 1.00,  # valor que você recebeu
        "endpoint": "/bootstrap-trust",  # ou "/starter-pack", etc.
        "payer": "payer-wallet-address"  # opcional
    }
    """
    tok = request.headers.get("X-Dash-Token") or request.args.get("token") or ""
    if tok != DASH_TOKEN:
    ...
    # Bypass replay check — limpa se existir
    h = hashlib.sha256(sig.encode()).hexdigest()
    LEDGER.replay_delete(h)

    # Registra diretamente no ledger
    LEDGER.add_revenue(endpoint, amount, sig, payer, source="manual-force", chain="solana")

    stats = LEDGER.stats()
    return jsonify({
        "success": True,
        "message": "Tx registrada manualmente no ledger!",
        "tx": sig[:32] + "...",
        "amount_usdc": amount,
        "endpoint": endpoint,
        "payer": payer,
        "new_total_usdc": stats["total_usdc"],
        "paid_24h": stats["paid_24h"],
        "warning": "Este registro bypassou verificação on-chain. Use apenas para txs que você confirmou no explorer."
    })


@app.route("/clear-replay", methods=["POST"])
def clear_replay():
    """v18: Limpa o replay cache para permitir re-registro de txs.

    Header: X-Dash-Token: <DASH_TOKEN>
    Body JSON: {"signatures": ["sig1", "sig2"]}
    """
    tok = request.headers.get("X-Dash-Token") or request.args.get("token") or ""
    if tok != DASH_TOKEN:
...
<div class="header">
  <div><h1>⚡ Losbeto <span class="badge">v18 REVENUE-FIX</span></h1>
...
<div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(250px,1fr))">
  <div class="card">
    ...
    <div class="sub" style="margin-top:8px">
      <a href="/register-marketplaces" target="_blank">⚡ Auto-Register (v17)</a> |
  ...
  <div class="card">
    <h3>🔍 Escanear TX (v18)</h3>
    <div class="sub" style="margin-top:8px">
      <code style="font-size:10px">GET /debug-tx?sig=&lt;signature&gt;</code>
      <div style="margin-top:6px">Diagnóstico COMPLETO — mostra por que passou/falhou</div>
      <div style="margin-top:4px"><a href="/debug-tx?sig=59Mn4RefaRf8ECRafYE7x4mTm2pkvJZGeekfVyzwyXLfA34eA1xdM7UzMq4RhNGv5pRXeuEG9B5mrt9tg9pviRvH" target="_blank">Testar com sua tx</a></div>
    </div>
  </div>
  <div class="card">
    <h3>🚨 Force Register (v18)</h3>
    <div class="sub" style="margin-top:8px">
      <code style="font-size:10px">POST /force-register-tx</code>
      <div style="margin-top:6px">Registra tx manualmente quando RPC falha</div>
      <div style="margin-top:4px;color:var(--red)">Requer X-Dash-Token</div>
    </div>
  </div>
  <div class="card">
...
<div class="footer">Atualização automática a cada 10s. Win-rate dirige preço (PoI). v18 REVENUE-FIX — <a href="/register-marketplaces" target="_blank">Registrar Marketplaces</a> • <a href="/health/deep" target="_blank">Deep Health</a> • <a href="/.well-known/x402.json" target="_blank">x402.json</a></div>
<script>
...
def run_server():
    ...
    log.info("=" * 72)
    log.info(f"⚡ LOSBETO v{VERSION} — ATIVO (REVENUE-FIX: vendas manuais corrigidas)")
