...
# ATENÇÃO: endereços Ethereum (0x...) não são válidos para USDC-SPL Solana.
# Use o endereço Solana da sua Binance (44 chars, sem 0x).
if _raw_binance.startswith("0x"):
    import sys as _sys
    _log_tmp = logging.getLogger("omega_v11")
    _log_tmp.error("⛔ BINANCE_SOLANA_ADDRESS começa com '0x' — isso é um endereço Ethereum!")
    _log_tmp.error("   Fundos enviados para esse endereço via Solana serão PERDIDOS.")
    _log_tmp.error("   Configure o endereço Solana da sua Binance (44 chars, sem 0x).")
    _raw_binance = ""  # Desativa sweep para proteger fundos
...
# Dynamic Pricing (PoI)
DYNAMIC_PRICING = os.environ.get("DYNAMIC_PRICING", "true").lower() == "true"
BASE_WIN_RATE   = 55.0
...
def _restore_wallets():
    ...
    # Proteção crítica: endereço público ≠ secret.
    # Um endereço Solana (ex: 9j3RRL...) decodifica para 32 bytes — é a chave
    # PÚBLICA. O secret (chave privada) também tem 32 bytes de seed mas gera
    # uma wallet COMPLETAMENTE DIFERENTE quando usado como SigningKey.
    # Para identificar se é endereço vs secret: tente gerar a wallet e veja
    # se o endereço resultante bate com o valor passado.
    if secret:
        try:
            raw = base58.b58decode(secret)
            if len(raw) not in (32, 64):
                log.error(f"⛔ WALLET_SECRET_B58 tamanho inválido: {len(raw)} bytes "
                          f"(esperado 32 ou 64).")
                secret = ""
            elif len(raw) == 32:
                # Pode ser endereço público ou seed — verifica se é o próprio endereço
                test_wallet = SolanaWallet(secret)
                if test_wallet.solana_address == secret:
                    log.error("⛔ WALLET_SECRET_B58 parece ser um ENDEREÇO PÚBLICO, não um secret!")
                    log.error("   Um endereço não pode ser usado como chave privada.")
                    log.error("   Para recuperar o secret, abra wallet.json e copie 'secret_b58'.")
                    log.error("   O secret é diferente do endereço e geralmente tem 64+ chars em base58.")
                    secret = ""
        except Exception as e:
            log.error(f"⛔ WALLET_SECRET_B58 inválido: {e}")
            secret = ""
    ...
    if not secret:
        ...
        log.warning(f"   Secret:  {w.export_b58_seed()}")
        log.warning(f"   Configure WALLET_SECRET_B58={w.export_b58_seed()} no Railway!")
    else:
        ...
        log.info(f"✅ Wallet restaurada: {w.solana_address}")
...
def _build_402(endpoint: str):
    """Resposta 402 spec-compliant — compatível com x402scan, AgentCash e AIDE.

    CRÍTICO: x402scan valida network="solana" (string simples).
    CAIP-2 "solana:5eykt4..." falha na maioria dos validadores.
    """
    amount = get_dynamic_price(endpoint)
    amount_atomic = str(int(amount * 10 ** USDC_DECIMALS))
    base = _public_base()
    ...
    if ENABLE_BASE and BASE_PAYTO_EVM:
        accepts.append({
            ...
            "maxAmountRequired": amount_atomic,
            "resource":          f"{base}{endpoint}",
            "description":       ENDPOINT_DESC.get(endpoint, "NexusOmega v11 — Base"),
            "mimeType":          "application/json",
            "payTo":             BASE_PAYTO_EVM,
            "maxTimeoutSeconds": 300,
            "asset":             BASE_USDC,
            "extra":             {"name": "USD Coin", "symbol": "USDC",
                                  "decimals": 6, "version": "2"},
        })
...
def manifest_x402():
    ...
    return jsonify({
        ...
        "payTo": {
            ...
            "base":   BASE_PAYTO_EVM if ENABLE_BASE else None,
            "ton":    TON_WALLET.address if TON_WALLET else None,
        },
        "resources": [
            {
                "url":      f"{base}{p}",
                ...
                "amount":   str(int(get_dynamic_price(p) * 10 ** USDC_DECIMALS)),
                "networks": (
        ...
        "instructions": "x402 v2. Use Coinbase x402, Corbits, PayAI, or MCPay clients. exact-scheme USDC on Solana mainnet. Multi-network: Solana + Base (if BASE_PAYTO_EVM set).",
    })
...
def openapi_spec():
    ...
    for p in BASE_PRICES:
        ...
        # Cada endpoint precisa ter pelo menos 1 parâmetro para o x402scan
        # não rejeitar com "Missing input schema"
        param = {
            "name": "format",
            "in": "query",
            "required": False,
            "description": "Formato da resposta (json)",
        ...
        paths[p] = {
            "get": {
                ...
                "description": (f"{ENDPOINT_DESC.get(p, p)}. "
                                f"Pague ${price:.4f} USDC-SPL via x402."),
    ...
    info = {
        ...
        "version":     VERSION,
        "description": "Multi-chain x402 AI swarm — Solana+Base. Pay-per-call USDC.",
    }
    if contact_email:
        ...
        "components": {
            "securitySchemes": {
                "x402": {
                    ...
                    "in":          "header",
                    "name":        "X-PAYMENT",
                    "description": "x402 payment header — base64 encoded payment payload",
                }
            }
        },
        "paths": paths,
...
<meta charset="utf-8"><title>NexusOmega v10 Dashboard</title>
...
<div class="header">
  <div><h1>⚡ NexusOmega <span class="badge">v10 SOVEREIGN</span></h1>
...
def autoregister_x402scan():
    """Registro automático no x402scan + AgentCash + verificação de duplicatas."""
    time.sleep(25)
    if not PUBLIC_URL:
        log.warning("autoregister: PUBLIC_URL não definida — configure no Railway Variables")
        return
    try:
        # 1. Verifica se o manifest está acessível externamente
        ...
        log.info("=" * 62)
        log.info(f"✅ NEXUS OMEGA v{VERSION} — nó público acessível")
        log.info(f"   URL:      {PUBLIC_URL}")
        log.info(f"   x402:     {PUBLIC_URL}/.well-known/x402.json")
        log.info(f"   MCP:      {PUBLIC_URL}/.well-known/mcp.json")
        log.info(f"   A2A:      {PUBLIC_URL}/.well-known/agent.json")
        log.info(f"   llms.txt: {PUBLIC_URL}/llms.txt")

        # 2. Tenta POST de auto-registro no x402scan
        try:
            reg = requests.post(
                "https://www.x402scan.com/api/register",
                json={"url": PUBLIC_URL, "version": 2},
                headers={"Content-Type": "application/json"},
                timeout=10
            )
            if reg.ok:
                log.info(f"✅ x402scan: auto-registro OK ({reg.status_code})")
            else:
                log.info(f"ℹ️  x402scan: registro manual necessário")
                log.info(f"   → https://www.x402scan.com/resources/register")
                log.info(f"   → Cole: {PUBLIC_URL}")
        except Exception:
            log.info(f"📍 x402scan: registre manualmente em:")
            log.info(f"   https://www.x402scan.com/resources/register")
            log.info(f"   URL: {PUBLIC_URL}")

        # 3. AgentCash — o principal cliente pagador do ecossistema
        log.info(f"📍 AgentCash (maior carteira de agentes x402):")
        log.info(f"   1. Acesse https://agentcash.dev")
        log.info(f"   2. Se houver listagem v1 antiga com endereço G383yQ..., DELETE-A")
        log.info(f"   3. Registre: {PUBLIC_URL}")
        log.info(f"   ⚠️  Listagem duplicada v1+v2 REDUZ visibilidade — delete a v1!")

        # 4. Wallet info
        log.info(f"💰 Recebe USDC-SPL em: {WALLET.solana_address}")
        if BINANCE_ADDRESS:
            log.info(f"🏦 Sweep → Binance Solana: {BINANCE_ADDRESS}")
        else:
            log.warning("⚠️  Sweep desativado. Configure BINANCE_SOLANA_ADDRESS com seu "
                        "endereço Solana da Binance (44 chars, sem 0x).")

        log.info("=" * 62)

        # 5. Agenda re-verificação em 10 minutos para confirmar registro
        def _recheck():
            time.sleep(600)
            try:
                r2 = requests.get(f"{PUBLIC_URL}/health", timeout=10)
                if r2.ok:
                    log.info("✅ Nó estável após 10min — pronto para receber pagamentos")
            except Exception:
                pass
