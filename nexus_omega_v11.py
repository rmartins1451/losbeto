...
def manifest_x402():
    ...
    CRÍTICO: usa BASE_PRICES (não dinâmico) para consistência com o paywall.
    CRÍTICO: network="solana" (string simples, não CAIP-2).
    """
    base = _public_base()
    resources = []
    ...
    return jsonify({
        ...
        "instructions": (
            ...
            "Chame sem header → receba 402 com paywall → pague → reenvie com X-PAYMENT."
        ),
    })
...
def openapi_spec():
    """OpenAPI com x-payment-info por operação — padrão que o x402scan usa para
    identificar endpoints como pagos (protegidos). Sem esse campo, o scanner
    classifica tudo como 'desprotegido' mesmo com 402 correto."""
    base = _public_base()
    ...
    for p in BASE_PRICES:
        ...
        paths[p] = {
            "get": {
                ...
                "parameters":  [param],
                "security":    [{"x402": []}],
                # ── x-payment-info: campo que x402scan lê para saber que este
                #    endpoint é pago. Sem ele → "endpoint desprotegido" ────────
                "x-payment-info": {
                    "scheme":            "exact",
                    "network":           "solana",
                    "maxAmountRequired": amount_atomic,
        ...
        "components": {
            "securitySchemes": {
                "x402": {
                    ...
                    "in":          "header",
                    "name":        "X-PAYMENT",
                    "description": "x402 payment header (base64 JSON PaymentPayload)",
                }
            }
        },
        "paths": paths,
