...
def _restore_wallets():
    ...
    # Se o usuário colou o endereço (44 chars, chave pública) no lugar
    # do secret (geralmente 64+ chars), a decodificação gerará uma wallet
    # DIFERENTE. Detectamos isso e alertamos claramente.
    if secret:
        try:
            raw = base58.b58decode(secret)
            if len(raw) not in (32, 64):
                log.error(f"⛔ WALLET_SECRET_B58 inválido: decodifica para {len(raw)} bytes "
                          f"(esperado 32 ou 64). Verifique se você colou o secret, "
                          f"não o endereço público.")
                secret = ""
        except Exception as e:
            log.error(f"⛔ WALLET_SECRET_B58 não é base58 válido: {e}")
            secret = ""
...
def _build_402(endpoint: str):
    """Resposta 402 spec-compliant x402 v2."""
